...
// ItemRepositoryCustom의 구현체에는 ---impl----을 붙여야 잘 작동함.
...
//   상품 판매 상태 조건이 전체이면(전체조회) null을 리턴, 이때 where절에서 해당 조건은 무시됨.
//   상태 조건이 있으면, 해당 조건을 사용하여 where구문이 만들어짐.
    private BooleanExpression searchSellSatuEq(ItemSellStatus searchSellSatus){
        return searchSellSatus == null ? null: QItem.item.itemSellStatus.eq(searchSellSatus);
    }
    ...
    public Page<Item> getAdminItemPage(ItemSearchDTO itemSearchDTO, Pageable pageable) {
        ...
        List<Item> result = queryFactory
                ...
                .where(regDtsAfter(itemSearchDTO.getSearchDateType()),
                        searchSellSatuEq(itemSearchDTO.getSearchSellStatus()),
                        searchByLike(itemSearchDTO.getSearchBy(),itemSearchDTO.getSearchQuery())
                      )
                .orderBy(QItem.item.id.desc()).offset(pageable.getOffset()) //offset() : 데이터를 가지고 올 시작 인덱스
