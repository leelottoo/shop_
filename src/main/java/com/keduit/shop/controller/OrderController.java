...
//        @ResponseBody : HTTP요청의 본문 Body에 담긴 내용을 자바 객체로 전달
...
//        principal : @Controller에노테이션이 선언된 클래스에서
//        메소드 인자로 pricipal을 넘겨줄 경우 해당 객체에 직접 접근이 가능하며,
//        pricipal객체에서 현재 로그인 한 회원의 이메일 정보를 조회함.
...
//    "/order/" + orderId + "/cancel"
    ...
    public @ResponseBody ResponseEntity cancelOrder(@PathVariable("orderId") Long orderId,Principal principal){
        ...
        if(!orderService.validateOrder(orderId, principal.getName())){  //로그인 사용자가 아닐 경우
            return new ResponseEntity("주문 취소 권한이 없습니다.", HttpStatus.FORBIDDEN); // HttpStatus.FORBIDDEN"권한이 없음
        }
