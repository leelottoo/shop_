...
public class CartService {
    ...
    public Long addCart(CartItemDTO cartItemDTO, String email) {
        ...
        //장바구니가 없으면 생성, 있으면 수량을 증가시킴
        if (cart == null) {
            cart = Cart.createCart(member);
            cartRepository.save(cart);
        }
        //장바구니에 상품이 들어 있는지 확인 후 있으면 수량 add, 없으면 장바구니 상품을 추가(save)하기
    ...
    public List<CartDetailDTO> getCartList(String email){
        ...
        Member member = memberRepository.findByEmail(email);  //로그인 한 사용자 모두
        Cart cart = cartRepository.findByMemberId(member.getId());
        if(cart == null){
...
//    현재 로그인 한 회원과 장바구니에 상품을 저장한 회원을 체크하여 같으면 true 다르면 false;
    ...
    public Long orderCartItem(List<CartOrderDTO> cartOrderDTOList, String email) {
        ...
        for(CartOrderDTO cartOrderDTO : cartOrderDTOList){
            ...
            orderDTO.setItemId(cartItem.getItem().getId()); //상품 아이디 Item 안에 Id 들어있음
